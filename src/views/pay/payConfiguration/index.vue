...
import {
  basePayConfigurationDeleteByIdSet,
  BasePayConfigurationDO,
  basePayConfigurationInfoById,
  basePayConfigurationInsertOrUpdate,
...
const search = ref<BasePayConfigurationPageDTO>({});
const searchRef = ref();

const loading = ref<boolean>(false);
const dataList = ref<BasePayConfigurationDO[]>([]);
...
function editClick(row: BasePayConfigurationDO) {
  title.value = "修改支付配置";
  formRef.value.editOpen(basePayConfigurationInfoById({ id: row.id }));
}

function addClick(row: BasePayConfigurationDO) {
  title.value = "新增支付配置";
...
function deleteClick(row: BasePayConfigurationDO) {
  ExecConfirm(
    async () => {
      await basePayConfigurationDeleteByIdSet({ idSet: [row.id] }).then(res => {
        ToastSuccess(res.msg);
        onSearch();
      });
    },
...
function deleteBySelectIdArr() {
  ...
  ExecConfirm(
    async () => {
      await basePayConfigurationDeleteByIdSet({
...
function onSelectChange(rowArr?: BasePayConfigurationDO[]) {
    ...
    <div class="flex flex-col px-5 py-3 bg-bg_color">
      ...
      >
        ...
        <el-table-column #default="scope" prop="type" label="类型">
          {{ BasePayTypeMap.get(scope.row.type) || "" }}
        </el-table-column>
