...
import {
  ...
  BaseApiTokenDO,
  baseApiTokenInfoById,
  baseApiTokenInsertOrUpdate,
  baseApiTokenPage,
  BaseApiTokenPageDTO
...
const search = ref<BaseApiTokenPageDTO>({});
const searchRef = ref();

const loading = ref<boolean>(false);
const dataList = ref<BaseApiTokenDO[]>([]);
...
function deleteBySelectIdArr() {
  ...
  ExecConfirm(
    async () => {
      await baseApiTokenDeleteByIdSet({ idSet: selectIdArr.value }).then(
        res => {
          ToastSuccess(res.msg);
          onSearch();
        }
