...
export const signUpTypeOptions: Array<OptionsType> = [];
export const tempRequestCategoryEnumMap = new Map<number, string>();

Object.keys(TempRequestCategoryEnum).forEach(key => {
  const item = TempRequestCategoryEnum[key] as IEnum<number>;

  tempRequestCategoryEnumMap.set(item.code, item.name);
  signUpTypeOptions.push({ label: item.name, value: item.code });
});
