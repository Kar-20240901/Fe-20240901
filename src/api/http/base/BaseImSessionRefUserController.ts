...
export interface ScrollListDTO {
  ...
  refId?: string; // 关联其他主键 id，format：int64
}

export interface BaseImSessionRefUserPageVO {
  lastContentType?: number; // 最新消息类型，format：int32
  lastContentCreateTime?: string; // 最新消息创建时间
  targetId?: string; // 目标 id，可以查询到好友用户主键 id，或者群组主键 id，一定有值，format：int64
  avatarUrl?: string; // 冗余字段：头像 url，一定有值
  lastContent?: string; // 最新消息，备注：会截断或者处理
  sessionName?: string; // 显示的会话名，一定有值
  targetType?: number; // 目标类型：101 好友 201 群组，一定有值，format：int32
  sessionId?: string; // 会话主键 id，一定有值，format：int64
  unReadCount?: number; // 未读数量，format：int32
}
